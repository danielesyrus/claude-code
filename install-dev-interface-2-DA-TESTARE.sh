...
function getMySQLCredentials() {
    ...
    if (!is_readable($credentialsFile) && $SUDO_ENABLED) {
        ...
        if ($result['status'] === 0) {
            ...
            return [
                'error' => 'Impossibile leggere il file delle credenziali'
            ];
        }
    } else {
    ...
    // Soluzione più robusta per estrarre le credenziali
    // Utilizziamo preg_match_all per catturare tutte le variabili nel formato KEY="VALUE"
    preg_match_all('/^\s*(DB_\w+)\s*=\s*"([^"]*)"\s*$/m', $content, $matches, PREG_SET_ORDER);
    ...
    // Verifica se abbiamo trovato le credenziali necessarie
    if (!isset($credentials['user']) || !isset($credentials['password'])) {
        // Seconda modalità di lettura - usando un comando shell dedicato
        if ($SUDO_ENABLED) {
